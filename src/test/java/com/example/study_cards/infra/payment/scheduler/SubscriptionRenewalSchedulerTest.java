package com.example.study_cards.infra.payment.scheduler;
...
class SubscriptionRenewalSchedulerTest extends BaseUnitTest {
    ...
    class CheckAndRenewSubscriptionsTest {
        ...
        void checkAndRenew_noBillingKey_skipsRenewal() {
            ...
            // then
            verify(tossPaymentService, never()).billingPayment(any(), any(), any(), anyInt(), any());
            verify(subscriptionDomainService, never()).createPayment(any(), any(), anyInt(), any());
        }
        ...
        void checkAndRenew_success() {
            ...
            given(subscriptionDomainService.findExpiredSubscriptions())
                    .willReturn(Collections.emptyList());
            given(subscriptionDomainService.createPayment(any(), any(), anyInt(), any()))
                    .willReturn(payment);
            ...
            verify(subscriptionDomainService).completePayment(eq(payment), any(), any());
        ...
        void checkAndRenew_failure_sendsNotification() {
            ...
            // then
            verify(subscriptionDomainService).failPayment(eq(payment), anyString());

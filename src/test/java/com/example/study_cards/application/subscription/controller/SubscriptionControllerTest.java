...
class SubscriptionControllerTest extends BaseIntegrationTest {
    ...
    @Nested
    @DisplayName("POST /api/subscriptions/checkout")
    class CheckoutTest {
        ...
        void checkout_success() throws Exception {
            ...
            mockMvc.perform(post("/api/subscriptions/checkout")
                    ...
                    .andDo(document("subscription/checkout",
                            preprocessRequest(prettyPrint()),
                            preprocessResponse(prettyPrint()),
                            requestHeaders(
                                    headerWithName("Authorization").description("Bearer 액세스 토큰")
    ...
    @Nested
    @DisplayName("POST /api/subscriptions/confirm")
    class ConfirmPaymentTest {
        ...
        void confirmPayment_success() throws Exception {
            ...
            String checkoutResponseBody = mockMvc.perform(post("/api/subscriptions/checkout")
                            .header("Authorization", "Bearer " + accessToken)
                            .contentType(MediaType.APPLICATION_JSON)
            ...
            mockMvc.perform(post("/api/subscriptions/confirm")
                            .header("Authorization", "Bearer " + accessToken)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(request)))
                    .andExpect(status().isOk())
                    ...
                    .andDo(document("subscription/confirm-payment",
                            preprocessRequest(prettyPrint()),
                            preprocessResponse(prettyPrint()),
                            requestHeaders(
                                    headerWithName("Authorization").description("Bearer 액세스 토큰")
    ...
    @Nested
    @DisplayName("GET /api/subscriptions/invoices")
    class GetPaymentHistoryTest {

        @Test
        @DisplayName("결제 내역을 조회한다")
        void getPaymentHistory_success() throws Exception {
            mockMvc.perform(get("/api/subscriptions/invoices")
                            .header("Authorization", "Bearer " + accessToken))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.content").isArray())
                    .andExpect(jsonPath("$.pageable").exists())
                    .andDo(document("subscription/get-invoices",
                            preprocessRequest(prettyPrint()),
                            preprocessResponse(prettyPrint()),
                            requestHeaders(
                                    headerWithName("Authorization").description("Bearer 액세스 토큰")
